package de.steallight.discordWhitelistPort.mcCMD;
...
public class MCWhitelist implements CommandExecutor, TabCompleter {
    ...
    MessageFormatter msgFormat = DiscordWhitelistPort.getPlugin().getMessageFormatter();
        ...
        if (sender instanceof Player) {
            if (cmd.getName().equalsIgnoreCase("wl")) {
                ...
                if (sender.isOp()) {
                        ...
                        switch (subcommand) {
                            case "add" -> {
                                if (!whitelistPlayer.isWhitelisted()) {
                                    ...
                                    addWhitelist(DiscordWhitelistPort.getPlugin().database, dcUserID, minecraftUsername);
                                    p.sendMessage(msgFormat.format(true, "success.whitelist-added"));
                                }else {
                                    p.sendMessage(DiscordWhitelistPort.getPlugin().getMessageFormatter().format(true, "whitelist.already-whitelisted"));
                                }
                            }
                            ...
                            case "remove" -> {
                                if (whitelistPlayer.isWhitelisted()) {
                                    ...
                                    removeWhitelist(DiscordWhitelistPort.getPlugin().database, minecraftUsername);
                    ...
                    p.sendMessage(DiscordWhitelistPort.getPlugin().getMessageFormatter().format(true, "error.no-permissions"));
                }
            }
        }
        return false;
